...
int bfm_sim_read_lepl1110(bfm_sim_t* sim, bfm_mesh_t* mesh, bfm_state_t* state, char* name) {
	...
	while (!feof(fp)) {
		...
		if (strncasecmp(line, "Type of problem	 ", 19) == 0) {
			...
			bfm_force_t* const force = state->alloc(sizeof *force);

			if (!force)
				goto err_while;

			if (bfm_force_create(force, state, 2) < 0)
				goto err_while;
	...
	if (bfm_material_create(material, state, "basic", E, rho, nu) < 0)
		goto err_mat;

	bfm_rule_t* const rule = state->alloc(sizeof *rule);
	...
	if (bfm_rule_create(rule, state, 2, mesh->kind, mesh->kind) < 0)
		goto err_mat;

	bfm_obj_t* const obj = state->alloc(sizeof *obj);
	...
	if (bfm_obj_create(obj, state, mesh, material, rule) < 0)
		goto err_mat;
	...
	if (bfm_instance_create(instance, state, obj) < 0)
		goto err_mat;
